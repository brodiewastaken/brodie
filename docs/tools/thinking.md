...
- Provider notes:
  ...
  - Fireworks keeps the five portable `/think low|medium|high|xhigh|max` levels and shows the effective provider mapping in each model's labels. Inkling passes all five levels through natively, maps off to `none`, and deliberately defaults to `high`. DeepSeek V4 and GLM 5.2 collapse the ladder to `high|max`; GPT-OSS and MiniMax M2.7 clamp `xhigh|max` to `high`; binary models map every non-off level to on. Kimi K3 maps `low → low`, `medium|high → high`, and `xhigh|max → max`, defaults to `max`, and preserves reasoning across tool-call replay. Fixed Fireworks Fast and US-only routers inherit the underlying model contract; moving `*-fast-latest` aliases are excluded because their target can change.

...
- `chat.history` returns authorized raw transcript rows in stable sequence order. Oversized rows use typed deferred references and bounded chunk retrieval, so the Control UI can expand the exact persisted row without a placeholder or display cap.
- Deferred oversized rows are fetched through `chat.message.get` by stable transcript sequence in fixed bounded byte chunks. The Control UI reconstructs UTF-8 across chunk boundaries, verifies the persisted SHA-256 digest, and fails visibly on missing or mismatched data.
...
- Control UI applies no standard display projection to `chat.history`. It renders the authorized raw sequence as semantic cards and keeps each exact persisted row available behind a disclosure.
...
- If live assistant text appears during a run but disappears after history reload, check whether the raw JSONL contains the assistant text and whether the Control UI optimistic-tail merge replaced local delivery state with the persisted snapshot.

...
<AccordionGroup>
  <Accordion title="Chat and Talk">
    ...
    - Chat history refreshes request a stable recent sequence page. Older rows prepend on demand, while oversized rows stay exact behind typed deferred references and bounded chunk reads instead of text caps or omission sentinels.
    - Typed channel turns expose one queue-first captured current-turn text item. Envelope-owned timestamps and media are not repeated before it, and runtime-only additions are shown at its tail instead of as a second user-role item.
...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - `chat.history` returns authorized raw transcript rows in stable sequence order. Oversized entries use typed deferred references and bounded chunk retrieval so the exact persisted row remains expandable without truncation.
    - Deferred oversized rows are fetched by stable transcript sequence through `chat.message.get` in fixed bounded byte chunks. The Control UI reconstructs UTF-8 across chunk boundaries, verifies the persisted SHA-256 digest, and shows missing chunks or hash mismatches as explicit failures.
    ...
    - The Control UI does not run a second display projection over history. It renders every authorized raw row in sequence as semantic inbound, thinking, assistant, tool, result, receipt, or runtime cards, with the exact persisted row available behind a disclosure.
    - New inbound rows distinguish the persisted envelope from captured current-turn text input. Typed scheduler turns show one literal queue-first current-user text item; envelope-owned timestamps, legacy media notes, and the former wrapped runtime-context tail item are absent, while unique runtime additions such as Slack thread history stay inside that item after the envelope. The disclosure is not a serialized provider request and does not duplicate system input, replay history, tool schemas, native images, or provider-specific transforms. Legacy rows omit this disclosure instead of guessing content that was never persisted.

...
- `channels.slack.thread.historyScope` is `"thread"` by default. `"thread"` isolates the rolling pending-history buffer per Slack thread; `"channel"` reuses the parent channel buffer. This setting does not change the initial Slack API fetch, which still uses `conversations.replies`.
- `channels.slack.thread.inheritParent` is `false` by default. When enabled, a new native thread session can fork the parent channel transcript when the parent exists and is within the transcript-size limit. This is transcript inheritance, not Slack history fetching.
- `channels.slack.thread.initialHistoryLimit` controls how many existing thread messages are retained when a missing or stale thread session starts (default `20`; set `0` to disable). Because Slack returns replies oldest-first, OpenClaw walks the complete `conversations.replies` cursor chain before keeping the newest configured number. If a later page fails or Slack repeats a malformed cursor, OpenClaw retains the pages already fetched and marks the restored history incomplete in model context instead of presenting the partial set as complete.
- When a missing or stale room-thread session is rebuilt, the configured fetched history retains both human messages and the bot's own previous replies so follow-ups preserve the available conversation. Before this behavior, every message matching the current bot user ID or bot ID was removed, whether it was a final answer, progress post, correction, or file-only post; only a bot-authored thread root could be retained as a special exception. Historical Slack attachments are downloaded into the private inbound media store after visibility filtering; each successful retained-file download receives the exact local path, while failed or oversized downloads retain an explicit unavailable file reference.

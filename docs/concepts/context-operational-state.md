...
Tool-result images externalized by Lossless are restored from the pre-assembly source snapshot by
exact tool-call identity. Direct image blocks and one-level native tool-result carriers are supported.
Restored images are placed at the outer tool-result content level for provider visibility, while the
externalized storage and marker text remain intact. Duplicate or ambiguous tool-call identities never
authorize another attachment. User and tool restoration share the configured image allowance.

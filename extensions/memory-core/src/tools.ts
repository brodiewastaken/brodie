...
import { readStringArrayParam } from "openclaw/plugin-sdk/param-readers";
...
const QMD_MEMORY_UNAVAILABLE_WARNING =
  "Tell the user: memory search is unavailable because the configured QMD backend failed.";

function buildQmdMemorySearchUnavailableResult(error: string | undefined, agentId: string) {
  return buildMemorySearchUnavailableResult(error, {
    warning: QMD_MEMORY_UNAVAILABLE_WARNING,
    action: `Tell the user to run: openclaw memory status --json --agent ${agentId}, then retry memory_search.`,
  });
}
  ...
  return createMemoryTool({
    ...
    execute:
      ...
      async (_toolCallId, params, callerSignal) => {
        ...
        const requestedCollections = readStringArrayParam(rawParams, "collections");
        ...
        if (requestedCollections?.length) {
          if (requestedCorpus === "wiki" || requestedCorpus === "sessions") {
            throw new Error(
              `memory_search collections cannot be used with corpus=${requestedCorpus}`,
            );
          }
          const { resolveMemoryBackendConfig } = await loadMemoryToolRuntime();
          const resolvedMemoryBackend = resolveMemoryBackendConfig({ cfg, agentId });
          if (resolvedMemoryBackend.backend !== "qmd" || !resolvedMemoryBackend.qmd) {
            throw new Error("memory_search collections require the QMD memory backend");
          }
          const configured = new Map(
            resolvedMemoryBackend.qmd.collections.map((collection) => [
              collection.name,
              collection.kind,
            ]),
          );
          const unknown = requestedCollections.filter((name) => !configured.has(name));
          if (unknown.length > 0) {
            throw new Error(`unknown QMD collection(s): ${unknown.join(", ")}`);
          }
          const expectedKind = requestedCorpus === "memory" ? "memory" : undefined;
          const incompatible = requestedCollections.filter((name) => {
            const kind = configured.get(name);
            return expectedKind === "memory" ? kind === "sessions" : false;
          });
          if (incompatible.length > 0) {
            throw new Error(
              `QMD collection(s) do not match corpus=${requestedCorpus}: ${incompatible.join(", ")}`,
            );
          }
        }
        ...
        let configuredMemoryBackend: "builtin" | "qmd" | undefined;
        ...
        ): Promise<T> => await runWithDeadline(DEFAULT_MEMORY_SEARCH_TIMEOUT_MS, task);
        const runMemorySearchTool = async () => {
          ...
          if (cooldown && !shouldQuerySupplements) {
            const { resolveMemoryBackendConfig } = await loadMemoryToolRuntime();
            configuredMemoryBackend = resolveMemoryBackendConfig({ cfg, agentId }).backend;
            return jsonResult(
              configuredMemoryBackend === "qmd"
                ? buildQmdMemorySearchUnavailableResult(cooldown.error, agentId)
          ...
          try {
              ...
              ? await runUnavailablePhase(
                  ...
                  async () =>
                    await runWithDefaultDeadline(async () => {
                      const { resolveMemoryBackendConfig } = await loadMemoryToolRuntime();
                      const resolvedMemoryBackend = resolveMemoryBackendConfig({ cfg, agentId });
                      configuredMemoryBackend = resolvedMemoryBackend.backend;
            ...
            if (shouldQueryMemory && memory && "error" in memory && !shouldQuerySupplements) {
              ...
              return jsonResult(
                configuredMemoryBackend === "qmd"
                  ? buildQmdMemorySearchUnavailableResult(memory.error, agentId)
              ...
              await runUnavailablePhase("memory", async () => {
                ...
                const searchActiveMemory = async (): Promise<MemorySearchResult[]> =>
                  await runWithDeadline(
                    // The configured backend owns the outer budget.
                    memorySetup.resolvedMemoryBackend.backend === "qmd"
                      ? (memorySetup.resolvedMemoryBackend.qmd?.limits.timeoutMs ??
                          DEFAULT_MEMORY_SEARCH_TIMEOUT_MS)
                      : DEFAULT_MEMORY_SEARCH_TIMEOUT_MS,
        ...
        try {
          ...
          return jsonResult(
            configuredMemoryBackend === "qmd"
              ? buildQmdMemorySearchUnavailableResult(message, agentId)

...
describe("QmdMemoryManager", () => {
  ...
  it("searches only requested managed collections and rejects unknown names", async () => {
    await configureMemoryCoreDreamingStateForTests();
    const otherWorkspaceDir = path.join(tmpRoot, "other-workspace");
    await fs.mkdir(otherWorkspaceDir, { recursive: true });
    cfg = {
    ...
    await expect(
      manager.search("default fact", {
        sessionKey: "agent:main:slack:dm:u123",
      }),
    ).resolves.toEqual([]);
    const defaultSearchCall = spawnMock.mock.calls.find(
      (call: unknown[]) =>
        (call[1] as string[])?.[0] === "search" && (call[1] as string[])?.[1] === "default fact",
    );
    expect(defaultSearchCall?.[1]).toContain("workspace");
    expect(defaultSearchCall?.[1]).not.toContain("other");
    ...
    const searchCall = spawnMock.mock.calls.find(
      (call: unknown[]) =>
        (call[1] as string[])?.[0] === "search" && (call[1] as string[])?.[1] === "fact",
    );
    expect(searchCall?.[1]).toContain("other");
    expect(searchCall?.[1]).not.toContain("workspace");
    expect(
      spawnMock.mock.calls.some(
        (call: unknown[]) => (call[1] as string[])?.join(" ") === "collection exclude other",
      ),
    ).toBe(true);
    const searchCallsBeforeUnknown = spawnMock.mock.calls.filter(
    ...
    ).rejects.toThrow("unknown QMD collection(s): missing");
    expect(
      spawnMock.mock.calls.filter((call: unknown[]) => (call[1] as string[])?.[0] === "search"),
    ).toHaveLength(searchCallsBeforeUnknown);
    await manager.close();
  });
  ...
  it("refreshes qmd index config with quoted collection values during update repair", async () => {
    ...
    expect(indexConfig).toContain("    includeByDefault: false");
    expect(updateCalls).toBe(2);
  ...
  it("parseShownCollection extracts path, pattern, and default inclusion", async () => {
    // Unit test for the private parser — accessed via type cast to avoid exporting internals.
    const { manager } = await createManager({ mode: "status" });
    type WithParser = {
      parseShownCollection: (output: string) => {
        path?: string;
        pattern?: string;
    ...
    expect(result.includeByDefault).toBe(true);
    ...
    expect(parser("Collection: opt-in\n  Include:  no")).toEqual({
      includeByDefault: false,
    });

...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private async ensureCollections(options?: {
    ...
    for (const collection of this.qmd.collections) {
      ...
      if (listed && !this.shouldRebindCollection(collection, listed)) {
        await this.ensureCollectionInclusion(collection, listed);
        continue;
      }
      if (listed) {
        try {
      ...
      try {
        ...
        await this.ensureCollectionInclusion(collection, existing.get(collection.name));
      } catch (err) {
        const message = formatErrorMessage(err);
  ...
  private async listCollectionsBestEffort(stats?: {
    ...
    for (const collection of this.qmd.collections) {
      ...
      try {
        ...
        if (shown.includeByDefault !== undefined) {
          entry.includeByDefault = shown.includeByDefault;
        }
      } catch {
  ...
  private async ensureCollectionInclusion(
    collection: ManagedCollection,
    listed?: ListedCollection,
  ): Promise<void> {
    if (
      listed?.includeByDefault === collection.includeByDefault ||
      (collection.includeByDefault && listed?.includeByDefault === undefined)
    ) {
      return;
    }
    await this.runQmd(
      ["collection", collection.includeByDefault ? "include" : "exclude", collection.name],
      { timeoutMs: this.qmd.update.commandTimeoutMs },
    );
    if (listed) {
      listed.includeByDefault = collection.includeByDefault;
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const entry of parsed) {
          ...
          const listedInclude = (entry as { includeByDefault?: unknown }).includeByDefault;
          listed.set(name, {
            ...
            pattern:
              typeof listedPattern === "string"
                ...
                : typeof listedMask === "string"
                  ? listedMask
                  : undefined,
            includeByDefault: typeof listedInclude === "boolean" ? listedInclude : undefined,
          });
        }
        return listed;
      }
  ...
  private parseShownCollection(output: string): ListedCollection {
    const result: ListedCollection = {};
    for (const rawLine of output.split(/\r?\n/)) {
      ...
      const includeMatch = /^\s*Include\s*:\s*(yes|no)\b/i.exec(rawLine);
      if (includeMatch) {
        result.includeByDefault = includeMatch[1].toLowerCase() === "yes";
      }
    }
    return result;
  }
  ...
  async search(
    ...
    const collectionNames = this.listManagedCollectionNames(requestedSources, opts?.collections);
  ...
  private listManagedCollectionNames(
    ...
    const sourceFiltered = !sources?.length
      ? this.managedCollectionNames
      : (() => {
    ...
    if (!requestedCollections?.length) {
      ...
      return sourceFiltered.filter((name) => defaults.has(name));
    }
    const requested = uniqueValues(requestedCollections.map((name) => name.trim()).filter(Boolean));
    const managed = new Set(this.managedCollectionNames);
    const unknown = requested.filter((name) => !managed.has(name));
    if (unknown.length > 0) {
      throw new Error(`unknown QMD collection(s): ${unknown.join(", ")}`);
    }
    const allowed = new Set(sourceFiltered);
    const incompatible = requested.filter((name) => !allowed.has(name));
    if (incompatible.length > 0) {
      throw new Error(
        `QMD collection(s) do not match the requested corpus: ${incompatible.join(", ")}`,
      );
    }
    return requested;
  }

...
function expectMemorySearchTimeout(
  ...
  expectUnavailableMemorySearchDetails(details, {
    ...
    action:
      backend === "qmd"
        ? "Tell the user to run: openclaw memory status --json --agent main, then retry memory_search."
...
describe("memory tool schemas", () => {
  it("uses flat corpus enums for provider tool compatibility", () => {
    ...
    expect(MemorySearchSchema.properties.collections).toMatchObject({
      type: "array",
      minItems: 1,
      uniqueItems: true,
    });
...
describe("memory_search unavailable payloads", () => {
  ...
  it("preserves qmd failures without suggesting an embedding rebuild", async () => {
    setMemoryBackend("qmd");
    setMemorySearchImpl(async () => {
      throw new Error("qmd query notes --json timed out after 45000ms");
    });

    const tool = createQmdTimeoutSearchTool();
    const result = await tool.execute("qmd-timeout", { query: "notes" });
    expectUnavailableMemorySearchDetails(result.details, {
      error: "qmd query notes --json timed out after 45000ms",
      warning:
        "Tell the user: memory search is unavailable because the configured QMD backend failed.",
      action:
        "Tell the user to run: openclaw memory status --json --agent main, then retry memory_search.",
  ...
  it("keeps qmd setup on the default deadline and closes a late one-shot manager", async () => {
    ...
    try {
      ...
      expectMemorySearchTimeout(result.details, 15, "qmd");
  ...
  it("pauses the configured qmd deadline around the command phase", async () => {
    vi.useFakeTimers();
    try {
      setMemoryBackend("qmd");
      let searchSignal: AbortSignal | undefined;
  ...
  it("counts qmd maintenance against the configured qmd deadline", async () => {
    vi.useFakeTimers();
    try {
      setMemoryBackend("qmd");
      let searchSignal: AbortSignal | undefined;
      ...
      const result = await resultPromise;
      expectMemorySearchTimeout(result.details, 45, "qmd");
      expect(searchSignal?.aborted).toBe(true);
    } finally {
      vi.useRealTimers();
    }
  ...
  it("uses the configured qmd deadline for qmd-configured managers", async () => {
    vi.useFakeTimers();
    try {
      ...
      await vi.advanceTimersByTimeAsync(QMD_SEARCH_TIMEOUT_MS - 1);
...
describe("memory_search corpus labels", () => {
  ...
  it("forwards exact configured QMD collection filters", async () => {
    ...
    const result = await tool.execute("scoped", {
      query: "current Ninebar ownership",
      corpus: "memory",
      collections: ["ninebar-live"],
    });

    expect(result.details).not.toMatchObject({ disabled: true });
    expect(searchOptions).toHaveLength(1);
    expect(searchOptions[0]?.collections).toEqual(["ninebar-live"]);
  });

  it("rejects collection filters on builtin memory without searching", async () => {
    let searchCalls = 0;
    setMemorySearchImpl(async () => {
      searchCalls += 1;
      return [];
    ...
    await expect(
      tool.execute("unsupported-collection", {
        query: "current Ninebar ownership",
        collections: ["ninebar-live"],
      }),
    ).rejects.toThrow("memory_search collections require the QMD memory backend");
    expect(searchCalls).toBe(0);
  });

  it("rejects unknown QMD collections without searching", async () => {
    let searchCalls = 0;
    setMemoryBackend("qmd");
    setMemorySearchImpl(async () => {
      searchCalls += 1;
    ...
    await expect(
      tool.execute("unknown-collection", {
        query: "current Ninebar ownership",
        collections: ["ninebar-typo"],
      }),
    ).rejects.toThrow("unknown QMD collection(s): ninebar-typo");
    expect(searchCalls).toBe(0);
  });
});

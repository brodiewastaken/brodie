...
describe("preflightDiscordMessage", () => {
  ...
  it("stores an owner bare control command for context in a mention-gated guild channel", async () => {
    const channelId = "channel-bare-control-command";
    const guildId = "guild-bare-control-command";
    const message = createDiscordMessage({
      id: "m-bare-control-command",
      channelId,
  ...
  it("routes prefixed guild text control commands through authorization instead of dropping them", async () => {
    const channelId = "channel-text-control-command";
    const guildId = "guild-text-control-command";
    const message = createDiscordMessage({
      id: "m-text-control-command",
      channelId,
      content: "brodie /steer keep digging",
      author: {
        id: "user-1",
        bot: false,
        username: "Alice",
    ...
    const preflight = expectPreflightResult(result);
    expect(preflight.baseText).toBe("brodie /steer keep digging");
    ...
    expect(preflight.addressed).toBe(false);
  });

...
describe("whatsapp inbound dispatch", () => {
  ...
  it("builds a finalized inbound context payload", async () => {
    const ctx = await buildWhatsAppInboundContext({
      ...
      msg: makeMsg({
        ...
        groupMention: { wasMentioned: false, requireMention: false, addressed: false },
        platform: {
          senderName: "Alice",
  ...
  it("never lets the owner command bypass authorize a bare /new in a larger group", async () => {
    const build = (params: { addressed: boolean; duoRoom?: boolean }) =>
      buildWhatsAppInboundContext({
        combinedBody: "Owner: /new",
        commandBody: "/new",
        commandAuthorized: true,
        commandTurn: { kind: "text-slash", source: "text", authorized: true, body: "/new" },
        groupHistory: [],
        groupMemberRoster: new Map(),
        msg: makeMsg({
          admission: {
            ...groupAdmission("123@g.us"),
            ...(params.duoRoom !== undefined ? { duoRoom: params.duoRoom } : {}),
          },
          payload: { body: "/new" },
          groupMention: { wasMentioned: true, requireMention: true, addressed: params.addressed },
          platform: {
            senderName: "Owner",
            senderJid: "owner@s.whatsapp.net",
            senderE164: "+15550001111",
          },
          group: { subject: "Test Group", participants: [] },
        }),
        route: makeRoute({ sessionKey: "agent:main:whatsapp:group:123@g.us" }),
        sender: { name: "Owner", e164: "+15550001111" },
      });

    const bypassed = requireRecord(await build({ addressed: false }), "bypassed");
    expect(bypassed.CommandAuthorized).toBe(false);
    expect(bypassed.WasMentioned).toBe(true);
    expect(String(bypassed.Body)).toContain("[UNAUTHORIZED COMMAND]");

    const addressed = requireRecord(await build({ addressed: true }), "addressed");
    expect(addressed.CommandAuthorized).toBe(true);
    expect(addressed.CommandBody).toBe("/new");

    const duo = requireRecord(await build({ addressed: false, duoRoom: true }), "duo");
    expect(duo.CommandAuthorized).toBe(true);
    expect(duo.CommandBody).toBe("/new");
  });

...
export function resolveTextCommandInvocation(params: {
  ...
  const bareAllowed =
    ...
    (params.conversationKind === "group" && params.duoRoom === true);
...
// Whether the text executes unaddressed: operator-name prefix, direct chat, or duo room.
export function isUnaddressedCommandExecutable(params: {
  cfg?: OpenClawConfig;
  agentId?: string;
  text: string;
  authorized: boolean;

        ...
        runWithModelFallback<EmbeddedAgentRunResult>({
          ...
          run: async (provider, model, runOptions) => {
            ...
            const candidateThinkLevel =
              runPolicy && isBrodieMaximumThinkingProvider(provider)
                ? resolveCandidateThinkingLevel({
                    cfg: runtimeConfig,
                    provider,
                    modelId: model,
                    level: runPolicy.reasoning,
                    agentId: params.followupRun.run.agentId,
                    sessionKey: params.followupRun.run.runtimePolicySessionKey ?? params.sessionKey,
                    sessionEntry: params.getActiveSessionEntry(),
                  })
                : ((runPolicy &&
                  (runPolicy.primary.provider !== provider || runPolicy.primary.model !== model)
                    ? resolveConfiguredFallbackThinking(runtimeConfig, provider, model)
                    : undefined) ??
                  runPolicy?.fallbackReasoning ??
                  runPolicy?.reasoning ??
                  resolveCandidateThinkingLevel({
                    cfg: runtimeConfig,
                    provider,

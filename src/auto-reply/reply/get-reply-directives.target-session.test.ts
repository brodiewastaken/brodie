...
vi.mock("../../agents/fast-mode.js", () => ({
  isFastModeEnforcedOff: (cfg: { agents?: { defaults?: { fastModeEnforcedOff?: boolean } } }) =>
...
describe("resolveReplyDirectives", () => {
  ...
  it("clamps a one-turn Fast override when the deployment locks Fast off", async () => {
    const { result } = await resolveHelloWithModelDefaults({
      defaultThinking: "off",
      defaultReasoning: "on",
      cfg: { agents: { defaults: { fastModeEnforcedOff: true } } },
      opts: { fastModeOverride: true },
    });

    expectContinueResult(result, { resolvedFastMode: false });
  });

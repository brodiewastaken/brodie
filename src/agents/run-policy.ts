...
import { isFastModeEnforcedOff, resolveFastModeState } from "./fast-mode.js";
...
export type RunPolicy = {
  ...
  /** Original reasoning intent retained for non-Go fallbacks from a forced Go primary. */
  fallbackReasoning?: ThinkLevel;
  fastMode: boolean;
  textVerbosity: "low";
...
const BRODIE_OPENCODE_GO_PRIMARY_REASONING: Readonly<Record<string, ThinkLevel>> = Object.freeze({
  "muse-spark-1.3-contributor": "xhigh",
  "deepseek-v4.1-flash": "max",
  "glm-5.3-flash": "max",
} satisfies Record<string, ThinkLevel>);

/** A fallback runs at its own model's configured thinking (`agents.defaults.models[ref].params.thinking`). */
export function resolveConfiguredFallbackThinking(
  cfg: OpenClawConfig,
  provider: string,
  model: string,
): ThinkLevel | undefined {
  const configured = cfg.agents?.defaults?.models?.[`${provider}/${model}`]?.params?.thinking;
  return typeof configured === "string" ? normalizeThinkLevel(configured) : undefined;
}

/** Returns whether brodie requires the highest supported thinking level for this provider. */
export function isBrodieMaximumThinkingProvider(provider: string): boolean {
...
export function resolveRunPolicy(params: {
  ...
  const resolvedFastMode =
    ...
    modelFast;
  const fastMode = isFastModeEnforcedOff(params.cfg) ? false : resolvedFastMode;
  ...
  const requestedReasoning =
    params.explicitReasoning ??
    configuredReasoning ??
    params.parent?.fallbackReasoning ??
    params.parent?.reasoning ??
    "high";
  const modelReasoning = isBrodieMaximumThinkingProvider(primary.provider)
    ? BRODIE_OPENCODE_GO_PRIMARY_REASONING[primary.model]
    : undefined;
  const reasoning = modelReasoning ?? requestedReasoning;
  const policy: RunPolicy = {
    primary,
    fallbacks: Object.freeze(fallbacks),
    reasoning,
    ...(modelReasoning ? { fallbackReasoning: requestedReasoning } : {}),
    fastMode,
    textVerbosity: "low",
        ...
        : params.explicitReasoning
          ...
          : configuredReasoning
            ...
            : params.parent
              ? "parent"
              : "default",
      fastMode: isFastModeEnforcedOff(params.cfg)
        ? "configured"

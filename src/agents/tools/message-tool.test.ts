...
describe("message tool conversational contract", () => {
  function createSingleRootSlackPlugin() {
        ...
        properties: {
          visibleMessages: Type.Optional(
            Type.Array(Type.String({ minLength: 1 }), {
              ...
              description:
                "Put the complete package in one visibleMessages item. If distinct standalone posts are intentional, use separate message tool calls.",
            }),
          ),
        },
        actions: ["send"],
  ...
  function conversationVariant(tool: ReturnType<CreateMessageTool>, action: string) {
    const schema = tool.parameters as {
      anyOf?: Array<{
        properties?: Record<string, unknown> & { action?: { const?: string } };
        required?: string[];
      }>;
    };
  ...
  it("rejects multi-part cross-route Slack channel sends before creating sibling roots", async () => {
    setActivePluginRegistry(
      createTestRegistry([
        { pluginId: "slack", source: "test", plugin: createSingleRootSlackPlugin() },
      ]),
    );
    const tool = createMessageTool({
      config: {} as never,
    ...
    await expect(
      tool.execute("1", {
        action: "send",
        channel: "slack",
        target: "channel:C0BSWB8B4PN",
        invisibleThinking: "handoff the complete project context",
        visibleMessages: ["mentioned root", "missing POC", "missing ownership map"],
        endTurn: false,
      }),
    ).rejects.toThrow(/one visibleMessages item.*separate message tool calls/i);
    expect(mocks.runMessageAction).not.toHaveBeenCalled();
  });

  it("rejects multi-part cross-route Slack direct sends before waking a DM bot early", async () => {
    setActivePluginRegistry(
      createTestRegistry([
        { pluginId: "slack", source: "test", plugin: createSingleRootSlackPlugin() },
      ]),
    ...
    await expect(
      tool.execute("1", {
        action: "send",
        channel: "slack",
        target: "user:U0BQJ20QCTY",
        invisibleThinking: "handoff the complete package",
  ...
  it("keeps intentional standalone Slack sends available as separate calls", async () => {
    setActivePluginRegistry(
      createTestRegistry([
        { pluginId: "slack", source: "test", plugin: createSingleRootSlackPlugin() },
      ]),
    );
    mockSendResult({ channel: "slack", to: "channel:C0BSWB8B4PN" });
    const tool = createMessageTool({
      config: {} as never,
      runMessageAction: mocks.runMessageAction as never,
    });
    ...
    for (const [index, message] of ["first", "second"].entries()) {
      await tool.execute(String(index), {
        action: "send",
        channel: "slack",
        target: "channel:C0BSWB8B4PN",
        invisibleThinking: "this is an intentional standalone post",
  ...
  it("accepts Slack topLevel for an explicit parent-channel send from a thread", async () => {
    ...
    mockSendResult({ channel: "slack", to: "channel:C0BSPDEKH6J" });
    const tool = createMessageTool({
      config: {} as never,
      currentChannelProvider: "slack",
      currentChannelId: "C0BSPDEKH6J",
      currentMessagingTarget: "channel:C0BSPDEKH6J",
      currentThreadTs: "1789096393.731569",
      runMessageAction: mocks.runMessageAction as never,
    });
    ...
    await tool.execute("1", {
      action: "send",
      channel: "slack",
      target: "channel:C0BSPDEKH6J",
      topLevel: true,
      invisibleThinking: "post a distinct parent-channel announcement",
      visibleMessages: ["root announcement"],
      endTurn: false,
    });
    ...
    expect(firstRunMessageActionInput()?.params).toMatchObject({
      ...
      target: "channel:C0BSPDEKH6J",
      topLevel: true,
      message: "root announcement",
    });
    const sendVariant = conversationVariant(tool, "send");
    const replyVariant = conversationVariant(tool, "reply");
    expect(sendVariant?.properties?.topLevel).toBeDefined();
    expect(sendVariant?.properties?.visibleMessages).toMatchObject({ maxItems: 9 });
    expect(replyVariant?.properties?.topLevel).toBeUndefined();
    expect(replyVariant?.properties?.visibleMessages).toMatchObject({ maxItems: 9 });
  });

  it("keeps other transport multi-bubble sends schema-valid in a Slack context", () => {
    setActivePluginRegistry(
      createTestRegistry([
        { pluginId: "slack", source: "test", plugin: createSingleRootSlackPlugin() },
      ]),
    ...
    expect(
      Value.Check(tool.parameters, {
        ...
        invisibleThinking: "two distinct beats belong in this route",
        visibleMessages: ["first", "second"],
        endTurn: false,
      }),
    ).toBe(true);
  ...
  it("names provider-supported actions when an action is unavailable", async () => {
    const slackPlugin = createChannelPlugin({
      id: "slack",
      label: "Slack",
      docsPath: "/channels/slack",
    ...
    mocks.runMessageAction.mockRejectedValueOnce(
      new Error("Action channel-info is not supported for provider slack."),
    );
    const tool = createMessageTool({
      config: {} as never,
      currentChannelProvider: "slack",
    ...
    await expect(
      tool.execute("1", {
        action: "channel-info",
        target: "C0BSWB8B4PN",
        invisibleThinking: "verify the destination",
      }),
    ).rejects.toThrow(
      "Action channel-info is not supported for provider slack. Supported actions: react, read, send.",
    );
...
describe("message tool schema scoping", () => {
  ...
  it("rejects discovered noncore actions on reply-only turns", () => {
    const plugin = createChannelPlugin({
      id: "imessage",
      label: "iMessage",
      docsPath: "/channels/imessage",
    ...
    setActivePluginRegistry(createTestRegistry([{ pluginId: "imessage", source: "test", plugin }]));
    const tool = createMessageTool({
      config: {} as never,
      currentChannelProvider: "imessage",
      sourceReplyDeliveryMode: "message_tool_only",
    ...
    expect(getActionEnum(getToolProperties(tool))).toContain("poll-vote");
    expect(() =>
      tool.prepareArguments?.({
        action: "poll-vote",
        invisibleThinking: "do not send a poll vote",
      }),
    ).toThrow(
      'message action "poll-vote" is unavailable this turn. this turn is reply-only: use action="reply" and omit channel and target; the current conversation is already bound.',
    );
  });

  it("requires terminal reply delivery on reply-only turns", () => {
    const tool = createMessageTool({
      config: {} as never,
      currentChannelProvider: "whatsapp",
      sourceReplyDeliveryMode: "message_tool_only",
    ...
    expect(() =>
      tool.prepareArguments?.({
        action: "reply",
        invisibleThinking: "send the saved result",
        visibleMessages: ["saved result"],
        endTurn: false,
      }),
    ).toThrow(

...
const ACTION_COPY: Record<CoreConversationalAction, string> = {
  ...
  send: "send is deliberate delivery to a different route and requires explicit channel and target. never use send to answer the current conversation. for a Slack DM or channel handoff, put the complete package in one visibleMessages item; multiple items can wake the recipient on a partial package and are rejected. if distinct standalone posts are intentional, use separate message tool calls.",
  react:
    "react applies visibleReaction natively to a message. when a reaction says enough, it is the whole response.",
  silence:
    "silence delivers nothing and ends the matter. silence always ends the turn, so omit endTurn. deciding not to speak is a first-class outcome, not a failure. record why in invisibleThinking.",
};

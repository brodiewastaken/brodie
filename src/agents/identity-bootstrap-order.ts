...
/**
 * Resolves the identity line that opens the system prompt. Set `identityLine` on the
 * identity-bootstrap hook entry to replace the default assistant line.
 */
export function resolveIdentityBootstrapLine(config?: OpenClawConfig): string | undefined {
  const internal = config?.hooks?.internal;
  if (internal?.enabled === false) {
    return undefined;
  }
  ...
  const line = typeof entry.identityLine === "string" ? entry.identityLine.trim() : "";

...
export function postprocessContextEngineMessages(params: {
  ...
  const maxImageInputCount =
    typeof params.maxImageInputCount === "number" &&
    Number.isSafeInteger(params.maxImageInputCount) &&
    params.maxImageInputCount >= 0
      ? params.maxImageInputCount
  ...
  const restored = restoreContextEngineImageBlocks({
    ...
    maxInputCount: maxImageInputCount,
    ...(params.sourceMessageLimit !== undefined
      ? { sourceMessageLimit: params.sourceMessageLimit }
      : {}),
  });
  const toolRestored = restoreContextEngineToolResultImageBlocks({
    sourceMessages: params.sourceMessages,
    assembledMessages: restored.messages,
    maxInputCount: Math.max(0, maxImageInputCount - restored.restoredCount),
    ...(params.sourceMessageLimit !== undefined
      ? { sourceMessageLimit: params.sourceMessageLimit }
      : {}),
  });
  const deduped = dedupeContextEngineQueueMediaMessages({ messages: toolRestored.messages });
  return {
    messages: deduped.messages,
    restoredCount: restored.restoredCount + toolRestored.restoredCount,
...
function getNestedToolResultCarrierContent(block: unknown): unknown[] | undefined {
  if (!isRecord(block) || (block.type !== "tool_result" && block.type !== "toolResult")) {
    return undefined;
  }
  return Array.isArray(block.content) ? block.content : undefined;
}

function getToolResultCarrierText(message: AgentMessage): string {
  const content = getMessageContent(message);
  if (!Array.isArray(content)) {
    return getMessageText(message);
  }
  const nestedText = content.flatMap((block) => {
    const nested = getNestedToolResultCarrierContent(block);
    return nested ? nested.flatMap((entry) => (isTextContentBlock(entry) ? [entry.text] : [])) : [];
  });
  return [getMessageText(message), ...nestedText].filter(Boolean).join("\n");
}

function getToolResultImages(message: AgentMessage): ImageContentBlock[] {
  const content = getMessageContent(message);
  if (!Array.isArray(content)) {
    return [];
  }
  return content.flatMap((block) => {
    ...
    const nested = getNestedToolResultCarrierContent(block);
    return nested?.flatMap((entry) => (isImageContentBlock(entry) ? [entry] : [])) ?? [];
  });
}

/**
 * Re-hydrates tool-result images only when Lossless preserved the exact tool
 * pairing and replaced the native block with its LCM externalization marker.
 */
function restoreContextEngineToolResultImageBlocks(params: {
  sourceMessages: AgentMessage[];
  assembledMessages: AgentMessage[];
  ...
  const sourceImagesByToolCallId = new Map<string, ImageContentBlock[] | null>();
  for (const message of params.sourceMessages.slice(0, sourceMessageLimit)) {
    const toolCallId = getToolResultCallId(message);
    if (!toolCallId) {
      continue;
    }
    const images = getToolResultImages(message);
    // A duplicate tool result id is malformed history. Do not guess which
    // image belongs to the assembled result when the pairing is ambiguous.
    sourceImagesByToolCallId.set(
      toolCallId,
      sourceImagesByToolCallId.has(toolCallId) || images.length === 0 ? null : images,
    );
  }

  if (sourceImagesByToolCallId.size === 0) {
  ...
  const messages = params.assembledMessages.map((message) => {
    if (
      remainingNativeImageSlots <= 0 ||
      message.role !== "toolResult" ||
      messageHasImageContent(message) ||
      !EXTERNALIZED_LCM_IMAGE_PATTERN.test(getToolResultCarrierText(message))
    ) {
      return message;
    }
    const toolCallId = getToolResultCallId(message);
    const images = sourceImagesByToolCallId.get(toolCallId ?? "");
    if (
      !toolCallId ||
      restoredToolCallIds.has(toolCallId) ||
      !images ||
      images.length > remainingNativeImageSlots
    ) {
      return message;
    }
    ...
    changed = true;
    return appendImageContentBlocks(message, images);
  });

  return { messages: changed ? messages : params.assembledMessages, restoredCount };
}
...
function toMidTurnPrecheckRequest(
  ...
  return {
    ...
    ...(result.toolResultAggregateMaxChars !== undefined
      ? { toolResultAggregateMaxChars: result.toolResultAggregateMaxChars }
      : {}),
    ...
    if (params.midTurnPrecheck?.enabled) {
      ...
      if (
        ...
        const heuristicPrecheck = shouldPreemptivelyCompactBeforePrompt({
          ...
          toolResultAggregateMaxChars: params.midTurnPrecheck.toolResultAggregateMaxChars,
        });

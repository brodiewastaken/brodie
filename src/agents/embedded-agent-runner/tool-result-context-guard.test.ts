...
function makeToolResultWithImage(id: string, text: string, imageData: string): AgentMessage {
  return castAgentMessage({
    role: "toolResult",
    toolCallId: id,
    toolName: "read",
...
function makeNestedToolResult(id: string, marker: string): AgentMessage {
  return castAgentMessage({
    role: "toolResult",
    toolCallId: id,
    toolName: "read",
...
function makeNestedToolResultWithImage(id: string, text: string, imageData: string): AgentMessage {
  return castAgentMessage({
    role: "toolResult",
    toolCallId: id,
    toolName: "read",
...
async function applyMidTurnPrecheckGuardToContext(
  ...
  installToolResultContextGuard({
    ...
    midTurnPrecheck: {
      ...
      toolResultAggregateMaxChars: options.toolResultAggregateMaxChars,
...
describe("restoreContextEngineImageBlocks", () => {
  it("rehydrates an externalized tool image only onto its paired tool result", () => {
    const imageCallId = "call_image|fc_000000000000000000000000000000000000000000000000";
    const userSource = makeUserWithImage("[media attached: media://inbound/user.jpg]", {
      data: "user-native-bytes",
    });
    const sourceTool = makeNestedToolResultWithImage(
      imageCallId,
      "Read image file [image/jpeg]",
      "tool-native-bytes",
    );
    const otherTool = makeToolResultWithImage(
      "call-other",
      "Read image file [image/jpeg]",
      "other-native-bytes",
    );
    const assembledUser = makeUser(
      "[User image: user.jpg (image/jpeg, 123 bytes) | LCM file: file_1111111111111111]",
    );
    const assembledTool = makeNestedToolResult(
      imageCallId,
      "[Tool image: tool-image.jpg (image/jpeg, 123 bytes) | LCM file: file_2222222222222222]",
    );
    const unpairedTool = makeToolResult(
      "call-missing",
      "[Tool image: missing-image.jpg (image/jpeg, 123 bytes) | LCM file: file_3333333333333333]",
      "read",
    );
    const duplicateAssembledTool = makeToolResult(
      imageCallId,
      "[Tool image: duplicate-image.jpg (image/jpeg, 123 bytes) | LCM file: file_4444444444444444]",
      "read",
    );

    const restored = postprocessContextEngineMessages({
      sourceMessages: [userSource, sourceTool, otherTool],
      assembledMessages: [assembledUser, assembledTool, unpairedTool, duplicateAssembledTool],
      maxImageInputCount: 3,
    });

    expect(restored.restoredCount).toBe(2);
    expect((restored.messages[0] as { content: unknown[] }).content).toContainEqual({
      type: "image",
      data: "user-native-bytes",
      mimeType: "image/jpeg",
    });
    const restoredToolContent = (restored.messages[1] as { content: unknown[] }).content;
    expect(restoredToolContent[0]).toEqual({
      type: "tool_result",
      tool_use_id: imageCallId,
      name: "read",
      content: [
        {
          type: "text",
          text: "[Tool image: tool-image.jpg (image/jpeg, 123 bytes) | LCM file: file_2222222222222222]",
        },
      ],
    });
    expect(restoredToolContent).toContainEqual({
      type: "image",
      data: "tool-native-bytes",
      mimeType: "image/jpeg",
    });
    expect((restored.messages[2] as { content: unknown[] }).content).not.toContainEqual({
      type: "image",
      data: "other-native-bytes",
      mimeType: "image/jpeg",
    });
    expect((restored.messages[3] as { content: unknown[] }).content).not.toContainEqual({
      type: "image",
      data: "tool-native-bytes",
      mimeType: "image/jpeg",
    });
  ...
  it("does not restore a tool image outside the selected source prefix", () => {
    const source = makeToolResultWithImage("call-image", "Read image file [image/jpeg]", "native");
    const assembled = makeToolResult(
      "call-image",
      "[Tool image: tool-image.jpg (image/jpeg, 123 bytes) | LCM file: file_2222222222222222]",
      "read",
    );
    ...
    const restored = postprocessContextEngineMessages({
      sourceMessages: [makeUser("earlier source"), source],
      assembledMessages: [assembled],
      sourceMessageLimit: 1,
    });

    expect(restored.restoredCount).toBe(0);
    expect(restored.messages).toEqual([assembled]);
  });

  it("restores the flat LCM tool-result assembly shape", () => {
    const source = makeToolResultWithImage("call-flat", "Read image file [image/jpeg]", "native");
    const assembled = makeToolResult(
      "call-flat",
      "[Tool image: tool-image.jpg (image/jpeg, 123 bytes) | LCM file: file_2222222222222222]",
      "read",
  ...
  it("spends the shared image cap on user restoration before tool restoration", () => {
    const sourceUser = makeUserWithImage("[media attached: media://inbound/user.jpg]", {
      data: "user-native",
    });
    const sourceTool = makeToolResultWithImage(
      "call-image",
      "Read image file [image/jpeg]",
      "tool-native",
    );
    ...
    const assembledTool = makeToolResult(
      "call-image",
      "[Tool image: tool-image.jpg (image/jpeg, 123 bytes) | LCM file: file_2222222222222222]",
      "read",
    );
    ...
    const restored = postprocessContextEngineMessages({
      sourceMessages: [sourceUser, sourceTool],
      assembledMessages: [assembledUser, assembledTool],
      maxImageInputCount: 1,
    });
    ...
    expect((restored.messages[1] as { content: unknown[] }).content).not.toContainEqual({
      type: "image",
      data: "tool-native",
      mimeType: "image/jpeg",
    });
...
describe("installToolResultContextGuard", () => {
  ...
  it("carries the authoritative aggregate repair cap through a mid-turn signal", async () => {
    const agent = makeGuardableAgent();
    const contextForNextCall = [
      makeUser("prompt already in history"),
      ...Array.from({ length: 6 }, (_, index) =>
        makeToolResult(`call_${index}`, "x".repeat(68_000)),
      ),
    ];
    ...
    try {
      await applyMidTurnPrecheckGuardToContext(agent, contextForNextCall, {
        ...
        prePromptMessageCount: 1,
        // The guard adds the 15-token rendered prompt envelope to this
        // assembled-context value before planning recovery.
        authoritativePromptTokens: 231_704,
      });
      throw new Error("expected mid-turn precheck signal");
    } catch (err) {
      expect(err).toBeInstanceOf(MidTurnPrecheckSignal);
      const signal = err as MidTurnPrecheckSignal;
      expect(signal.request.route).toBe("truncate_tool_results_only");
      expect(signal.request.overflowTokens).toBe(1_719);
      expect(signal.request.toolResultAggregateMaxChars).toBe(397_686);
    }
  });

...
describe("runEmbeddedAgent incomplete-turn safety", () => {
  ...
  it("forces OpenCode Go thinking at the final embedded model boundary", async () => {
    ...
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      makeAttemptResult({ assistantTexts: ["completed"] }),
    );
    ...
    await runEmbeddedAgent({
      ...
      thinkLevel: "off",
      runId: "run-opencode-go-final-thinking",
    });

    expect(mockedRunEmbeddedAttempt).toHaveBeenCalledOnce();
    expect(mockedRunEmbeddedAttempt.mock.calls[0]?.[0]).toMatchObject({ thinkLevel: "max" });
  });

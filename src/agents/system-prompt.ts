...
export function buildAgentSystemPrompt(params: {
  ...
  /** Opening identity line; the identity-bootstrap hook supplies it, else the default assistant line. */
  identityLine?: string;
  bootstrapMode?: BootstrapMode;
  bootstrapTruncationNotice?: string;
  skillsPrompt?: string;
  ...
  const identityLine =
    params.identityLine?.trim() || "You are a personal assistant running inside OpenClaw.";
  // For "none" mode, return just the basic identity line
  if (promptMode === "none") {
    return [identityLine, modelIdentityLine].filter(Boolean).join("\n");
  }

...
describe("resolveIdentityBootstrapLine", () => {
  it("returns the trimmed identity line only while the hook is enabled", () => {
    expect(resolveIdentityBootstrapLine(undefined)).toBeUndefined();
    expect(
      resolveIdentityBootstrapLine(configWithHookEntry({ identityLine: "YOU ARE BRODIE." })),
    ).toBeUndefined();
    expect(
      resolveIdentityBootstrapLine(
        configWithHookEntry({ enabled: true, identityLine: "  YOU ARE BRODIE.  " }),
      ),
    ).toBe("YOU ARE BRODIE.");
    expect(
      resolveIdentityBootstrapLine(configWithHookEntry({ enabled: true, identityLine: "   " })),
    ).toBeUndefined();
    expect(
      resolveIdentityBootstrapLine(configWithHookEntry({ enabled: true, identityLine: 7 })),
    ).toBeUndefined();
  });

  it("returns undefined when the hooks master switch is off", () => {
    const config = configWithHookEntry({ enabled: true, identityLine: "YOU ARE BRODIE." });
    config.hooks!.internal!.enabled = false;
    expect(resolveIdentityBootstrapLine(config)).toBeUndefined();
  });
});

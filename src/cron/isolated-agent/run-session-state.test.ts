...
describe("createPersistCronSessionEntry", () => {
  it.each([false, true])(
    "preserves an exact-run model change and retired policy through later writes (default=%s)",
    async (isDefault) => {
      const stableKey = "agent:main:cron:job";
      const runKey = `${stableKey}:run:run-session-id`;
      ...
      const persist = createPersistCronSessionEntry({
        isFastTestEnv: false,
        cronSession,
        agentSessionKey: stableKey,
        runSessionKey: runKey,
        applySessionEntryLifecycleMutation: adaptUpdateSessionStore(async (_path, update) => {
          update(store);
        }),
      });
      await persist();
      ...
      applyModelOverrideToSessionEntry({
        entry: store[runKey],
        selection: { provider: "openai", model: "gpt-5.5", isDefault },
        markLiveSwitchPending: true,
      });
      expect(store[runKey].cronRunContinuationPolicy).toBeUndefined();

      cronSession.sessionEntry.status = "done";
      await persist();
      await persist();

      expect(store[runKey]).toMatchObject({ status: "done" });
      expect(store[runKey].cronRunContinuationPolicy).toBeUndefined();
      expect(store[runKey].providerOverride).toBe(isDefault ? undefined : "openai");
      expect(store[runKey].modelOverride).toBe(isDefault ? undefined : "gpt-5.5");
      expect(store[stableKey].cronRunContinuationPolicy?.model).toBe("grok-4.6");
    },
  );

...
function isRunnableJob(params: {
  ...
  return previousRunAtMs > resolveMissedSlotBaselineMs(job, lastRunAtMs);
}

/**
 * A slot only counts as missed when it is later than both the last real run
 * and the last scheduling-input change. `lastRunAtMs` was recorded under the
 * schedule live at the time; after a tz or expression edit the current
 * schedule's previous slot can land after that run without ever having been
 * due (the Rakuten weekly check-in replayed 19 hours early this way on
 * 2026-09-17). Slots after the edit are still covered: `nextRunAtMs` is
 * recomputed at edit time and the primary due check handles real downtime.
 */
function resolveMissedSlotBaselineMs(job: CronJob, lastRunAtMs: number): number {
  const changedAtMs = job.state.scheduleChangedAtMs;
  if (typeof changedAtMs !== "number" || !Number.isFinite(changedAtMs)) {
    return lastRunAtMs;
  }
  return Math.max(lastRunAtMs, changedAtMs);
}
...
function deferPendingBackoffMissedCronSlots(
  ...
  for (const job of state.store.jobs) {
    ...
    if (
      ...
      previousRunAtMs <= resolveMissedSlotBaselineMs(job, lastRunAtMs)
    ) {
      continue;
    }

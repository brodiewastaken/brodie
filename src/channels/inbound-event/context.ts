...
/**
 * A channel that separates platform addressing from its control-command
 * bypass reports `addressed`; only a channel that cannot make that split
 * falls back to `wasMentioned`.
 */
function resolveAddressedFact(mentions: AccessFacts["mentions"]): boolean {
  if (!mentions) {
    return false;
  }
  if (mentions.addressed !== undefined) {
    return mentions.addressed;
  }
  return mentions.wasMentioned;
}
...
export function buildChannelInboundEventContext(
  ...
  const invocation = resolveTextCommandInvocation({
    ...
    addressed: resolveAddressedFact(params.access?.mentions),
    native: commandTurn?.kind === "native",
    conversationKind: params.conversation.kind,
    duoRoom: params.conversation.duoRoom === true,
  });

...
describe("runtime conversation scheduler", () => {
  ...
  it("binds a debounced batch to the latest trusted reply target", async () => {
    const batch = dispatchBatch();
    const firstReplyTarget = {
      channel: "slack",
      accountId: "brodie",
      target: "channel:C123",
      threadId: "thread-1",
    ...
    batch.events = [
      {
        ...scheduled("1", "first"),
        route: { ...route, currentReplyTarget: firstReplyTarget },
      },
      {
        ...scheduled("2", "second"),
        route: { ...route, currentReplyTarget: latestReplyTarget },
      },
    ];
    ...
    await runtimeConversationSchedulerTesting.dispatchOwnedBatchWithReply({
      ...
      resolveTypingCallbacks,
      deliverReturnedReply,
      getReply: async (ctx, opts) => {
        expect(ctx.MessageSid).toBe("message-2");
        opts.onAgentRunStart?.("attempt");
        return { text: "terminal failure", isAgentRunFailure: true };
      },
    });

    expect(resolveTypingCallbacks).toHaveBeenCalledWith({
      cfg: {},
      target: latestReplyTarget,
    });
    expect(deliverReturnedReply).toHaveBeenCalledWith(
      expect.objectContaining({
        target: latestReplyTarget,
        route: expect.objectContaining({ currentReplyTarget: latestReplyTarget }),
      }),
    );
  });
  ...
  it("rejects mismatched terminal evidence and replays only a proven never-started run", async () => {

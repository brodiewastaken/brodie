...
describe("ConversationScheduler", () => {
  it("correlates durable admission and dispatch wait with the scheduler receipt and run", async () => {
    const { route: baseRoute, database } = await fixture();
    const route = directRoute(baseRoute);
    const events: DiagnosticEventPayload[] = [];
    setDiagnosticsEnabledForProcess(true);
    onDiagnosticEvent((diagnostic) => events.push(diagnostic));
    ...
    const admitted = await scheduler.admit(event(route, "timed-admission"));
    if (!admitted.accepted) {
      throw new Error("expected timed scheduler admission");
    }
    await eventually(() =>
      events.some((entry) => entry.type === "queue.lane.dequeue" && entry.runId !== undefined),
    );
    await eventually(
      async () => (await scheduler.snapshot(route)).lanes[0]?.activeState === undefined,
    );

    expect(events).toContainEqual(
      expect.objectContaining({
        type: "queue.lane.enqueue",
        lane: route.queueLaneKey,
        sessionKey: route.sessionKey,
        receiptId: admitted.receiptId,
        producerKind: "human_message",
        admissionDelayMs: 100,
      }),
    );
    expect(events).toContainEqual(
      expect.objectContaining({
  ...
  it("replays a proven never-started durable event without duplicating its identity", async () => {
    const { route: baseRoute, database } = await fixture();
    const route = directRoute(baseRoute);
    const scheduler = createConversationScheduler({ database });
    const admitted = await scheduler.admit(event(route, "never-started"));
    if (!admitted.accepted) {
      throw new Error("expected never-started admission");
    }
    const { db } = openOpenClawStateDatabase(database);
    db.prepare(
      `UPDATE conversation_scheduler_events
       SET state = 'running', dispatch_attempt_id = 'attempt-never-started',
           run_correlation_id = 'run-never-started'
       WHERE event_id = 'never-started'`,
    ).run();
    db.prepare(
      `UPDATE conversation_scheduler_lanes SET active_event_id = 'never-started'
       WHERE lane_key = ?`,
    ).run(route.queueLaneKey);
    const batches: SchedulerDispatchBatch[] = [];
    const recovered = createConversationScheduler({
    ...
    expect(
      db
        .prepare(
          `SELECT COUNT(*) AS count FROM conversation_scheduler_events
           WHERE event_id = 'never-started'`,
        )
        .get(),
    ).toEqual({ count: 1 });
  ...
  it.each(["live", "unresolved"] as const)(
    "replays a never-started attempt when a later pending check resolves %s startup evidence",
    async (initialStatus) => {
      const { route: baseRoute, database } = await fixture();
      const route = directRoute(baseRoute);
      const original = createConversationScheduler({ database });
      const admitted = await original.admit(event(route, "late-replay"));
      if (!admitted.accepted) {
      ...
      db.prepare(
        `UPDATE conversation_scheduler_events
         SET state = 'running', dispatch_attempt_id = 'late-attempt',
             run_correlation_id = 'late-run'
         WHERE event_id = 'late-replay'`,
      ).run();
      db.prepare(
        `UPDATE conversation_scheduler_lanes SET active_event_id = 'late-replay'
         WHERE lane_key = ?`,
      ).run(route.queueLaneKey);
      const batches: SchedulerDispatchBatch[] = [];
      let checks = 0;
      const recovered = createConversationScheduler({
        ...
        reconcileInterruptedAttempt: async () =>
          ++checks === 1
            ...
            : { status: "replayable", evidence: { kind: "source_run_never_started" } },
        dispatch: async (batch) => {
          batches.push(batch);
          return delivered(batch);
        },
      ...
      await eventually(async () =>
        ["delivered", "failed"].includes(
          (await recovered.waitForReceiptTerminal?.(admitted.receiptId)) ?? "",
        ),
      );
      expect(await recovered.waitForReceiptTerminal?.(admitted.receiptId)).toBe("delivered");
      expect(batches).toHaveLength(1);
      expect(batches[0]?.events).toEqual([
        expect.objectContaining({ id: "late-replay", receiptId: admitted.receiptId }),
      ]);
    },
  );

  it("keeps started-run evidence on later pending reconciliation", async () => {
    const { route: baseRoute, database } = await fixture();
    const route = directRoute(baseRoute);
    const original = createConversationScheduler({ database });
    const admitted = await original.admit(event(route, "late-started"));
    if (!admitted.accepted) {
      throw new Error("expected durable admission");
    }
    const { db } = openOpenClawStateDatabase(database);
    db.prepare(
      `UPDATE conversation_scheduler_events
       SET state = 'running', dispatch_attempt_id = 'started-attempt',
           run_correlation_id = 'started-run', transcript_evidence = 'started-evidence'
       WHERE event_id = 'late-started'`,
    ).run();
    db.prepare(
      `UPDATE conversation_scheduler_lanes SET active_event_id = 'late-started'
       WHERE lane_key = ?`,
    ).run(route.queueLaneKey);
    const observedEvidence: Array<string | undefined> = [];
    const batches: SchedulerDispatchBatch[] = [];
    const recovered = createConversationScheduler({
      database,
      reconcileIntervalMs: 1,
      reconcileInterruptedAttempt: async (attempt) => {
        observedEvidence.push(attempt.transcriptEvidence);
        return observedEvidence.length === 1
          ? { status: "live" }
          : { status: "retryable", evidence: { kind: "started_run_interrupted" } };
      },
      dispatch: async (batch) => {
        batches.push(batch);
        return delivered(batch);
    ...
    await eventually(
      async () => (await recovered.waitForReceiptTerminal?.(admitted.receiptId)) === "failed",
    );
    expect(observedEvidence).toEqual(["started-evidence", "started-evidence"]);
    expect(batches).toEqual([]);
  });

  it("delivers a durable child completion after a started parent is interrupted", async () => {
    const { route: baseRoute, database } = await fixture();
    const route = directRoute(baseRoute);
    const scheduler = createConversationScheduler({ database });
    await scheduler.admit(event(route, "started-parent"));
    const child = await scheduler.admit({
      ...event(route, "completed-child", { taskRunId: "child-run-1" }),
      producerKind: "subagent_completion",
      human: false,
    });
    if (!child.accepted) {
      throw new Error("expected completed child admission");
    }
    const { db } = openOpenClawStateDatabase(database);
    db.prepare(
      `UPDATE conversation_scheduler_events
       SET state = 'running', dispatch_attempt_id = 'attempt-started-parent',
           run_correlation_id = 'run-started-parent',
           transcript_evidence = 'session:started'
       WHERE event_id = 'started-parent'`,
    ).run();
    db.prepare(
      `UPDATE conversation_scheduler_lanes SET active_event_id = 'started-parent'
       WHERE lane_key = ?`,
    ).run(route.queueLaneKey);
    const batches: SchedulerDispatchBatch[] = [];
    createConversationScheduler({
    ...
    expect(
      db
        .prepare(
          `SELECT event_id, state FROM conversation_scheduler_events
           WHERE event_id IN ('started-parent', 'completed-child') ORDER BY event_id`,
        )
        .all(),
    ).toEqual([
      { event_id: "completed-child", state: "delivered" },
      { event_id: "started-parent", state: "failed" },
    ]);
  });

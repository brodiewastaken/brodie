...
import { logLaneDequeue, logLaneEnqueue } from "../logging/diagnostic-runtime.js";
...
function queryLanePendingCount(db: DatabaseSync, laneKey: string): number {
...
class SqliteConversationScheduler implements ConversationScheduler {
  ...
  async admit(event: ScheduledEvent): Promise<AdmissionResult> {
    ...
    let admittedQueueSize: number | undefined;
    const result = await this.coordinator.run(event.route.queueLaneKey, () => {
      try {
        const admitted = runOpenClawStateWriteTransaction(({ db }) => {
          ...
          admittedQueueSize = queryLanePendingCount(db, event.route.queueLaneKey);
    ...
    if (result.accepted && result.existingState === undefined) {
      logLaneEnqueue(event.route.queueLaneKey, admittedQueueSize ?? 1, {
        sessionKey: event.route.sessionKey,
        receiptId: result.receiptId,
        producerKind: event.producerKind,
        admissionDelayMs: Math.max(0, result.durableAt - event.createdAt),
      });
    }
  ...
  private async pumpLane(laneKey: string): Promise<void> {
    ...
    const first = reservation.rows[0];
    if (first) {
      logLaneDequeue(
        reservation.laneKey,
        Math.max(0, reservation.reservedAt - first.durable_at),
        reservation.remainingQueueSize,
        {
          sessionKey: first.session_key,
          runId: reservation.attemptId,
          producerKind: first.producer_kind,
          receiptCount: reservation.rows.length,
        },
      );
    }
  ...
  private reserveReadyBatch(laneKey: string): Reservation | undefined {
    return runOpenClawStateWriteTransaction(({ db }) => {
      ...
      return {
        ...
        rows,
        reservedAt: now,
        remainingQueueSize: queryLanePendingCount(db, laneKey),
      };
    }, this.options.database);
  }
    ...
    const updated = executeSqliteQuerySync(
      ...
      kysely
        ...
        .set({
          ...
          run_correlation_id: null,
          transcript_evidence: null,
          failure_json: JSON.stringify({ kind: "restart_replayable", evidence }),
          revision: sql`revision + 1`,
          updated_at: now,
        })
        .where("lane_key", "=", laneKey)
        .where("dispatch_attempt_id", "=", attemptId)
  ...
  private async reconcilePendingLane(laneKey: string): Promise<void> {
    ...
    try {
      ...
      const reservation: Reservation = {
        ...
        placement: "recovery",
        rows,
        reservedAt: this.options.now?.() ?? Date.now(),
      ...
      const reconciliation = await reconcile({
        ...
        ...(first.transcript_evidence ? { transcriptEvidence: first.transcript_evidence } : {}),
      ...
      if (reconciliation.status === "replayable") {
        await this.coordinator.run(laneKey, () =>
          runOpenClawStateWriteTransaction(({ db: tx }) => {
            this.replayInterruptedRows(tx, laneKey, reservation.attemptId, reconciliation.evidence);
          }, this.options.database),
        );
        this.coldPendingGraceRemaining.delete(reservation.attemptId);
      } else if (reconciliation.status === "delivered") {
        await this.finishReservation(reservation, {
          outcome: "completed",
      ...
      for (const [laneKey, rows] of interruptedByLane) {
        ...
        await this.coordinator.run(laneKey, () => {
          runOpenClawStateWriteTransaction(({ db: tx }) => {
            ...
            if (reconciliation.status === "replayable") {
              this.replayInterruptedRows(tx, laneKey, attemptId, reconciliation.evidence);
              return;
            }
            const state =
